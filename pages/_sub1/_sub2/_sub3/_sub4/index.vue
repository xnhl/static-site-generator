<template>
	<div id="page">
		<Menu :files="files" />
		<div id="content">
			<Heading :title="`${this.$route.params.sub1} / ${this.$route.params.sub2} / ${this.$route.params.sub3} / ${this.$route.params.sub4}`" />
...
export default {
	...
	async asyncData({ route, params, redirect, $content }) {
		try {
			const info = await $content(params.sub1, params.sub2, params.sub3, params.sub4).fetch()
			const files = await $content({ deep: true }).only(['path']).fetch()
			const [prev, next] = await $content(params.sub1, params.sub2, params.sub3).only(['title', 'slug']).sortBy('createdAt', 'asc').surround(params.sub4).fetch()
			if (prev !== null) { prev.pre = `${params.sub1}/${params.sub2}/${params.sub3}` }
			if (next !== null) { next.pre = `${params.sub1}/${params.sub2}/${params.sub3}` }
			return {
				info,
				files,
				prev,
